...
Page({
    ...
    getData: function(){
        utils.request(api.getStoreEntryData).then((res)=>{
    ...
    formSubmit({detail:{value}}) {
        const { params,storefrontList,gpsLng,gpsLat,storeInfoList, morningTimeStart,morningTimeEnd,afternoonTimeStart,afternoonTimeEnd,region,districtCode } = this.data;
        if(!districtCode){
            wx.showToast({
                icon: 'none',
                title: '请选择门店地址'
        ...
        wx.setStorageSync("storeEntryData",{
